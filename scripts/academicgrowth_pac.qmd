...
```{r}
df_wide <- pivot_wider(data2, names_from = year, values_from = c(RC_growth, LWID_growth, WA_growth, RF_growth, WF_growth, WS_growth, CALC_growth, MF_growth))
df_wide
...
copy_df$reading <- (copy_df$RC_growth + copy_df$LWID_growth + copy_df$RF_growth) / 3
copy_df$reading <- round(copy_df$reading, 2)

copy_df$writing <- (copy_df$WA_growth + copy_df$WF_growth + copy_df$WS_growth) / 3
copy_df$writing <- round(copy_df$writing, 2)

copy_df$math <- (copy_df$CALC_growth + copy_df$MF_growth) / 2
copy_df$math <- round(copy_df$math, 2)
```

```{r}
mean_reading <- mean(copy_df$reading, na.rm = TRUE)
mean_reading <- round(mean_reading, 2)
mean_reading

mean_writing <- mean(copy_df$writing, na.rm = TRUE)
mean_writing <- round(mean_writing, 2)

mean_math <- mean(copy_df$math, na.rm = TRUE)
mean_math <- round(mean_math, 2)
```

```{r}
reading_by_year <- copy_df %>% 
  group_by(year) %>% 
  summarise(mean_reading = mean(reading, na.rm = TRUE))

reading_by_year

writing_by_year <- copy_df %>% 
  group_by(year) %>% 
  summarise(mean_writing = mean(writing, na.rm = TRUE))

writing_by_year

math_by_year <- copy_df %>% 
  group_by(year) %>% 
